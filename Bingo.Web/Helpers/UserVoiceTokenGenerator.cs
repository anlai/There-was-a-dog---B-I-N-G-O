...
namespace Bingo.Web.Helpers
{
    ...
    {
        public static string CreateWithCurrentUserForTesting(IPrincipal user)
        {
            //Just email everything to automatedemail, and use the current user kerb for display
            return Create(user.Identity.Name, "automatedemail@caes.ucdavis.edu", user.Identity.Name);
        }

        public static string Create(string displayName, string email, string guid, int minutesUntilExpiration = 2)
        {
